# Как подключить сервер к маку для компилляции x86 программ
...
# Будут предложены настройки конфигов, выбираем самый первый(фото не могу подставить)
...
# Качаем все расширения для Vscoda, которые были, тк они не переносятся
